...
import { Github, ArrowRight, Code2, Layers } from 'lucide-react';
import { projects } from '../../data/projects';

const Projects = () => {
  // Set Nexus as the featured project (id: 2)
  ...
  const featuredDetails = {
    problem: "Managing multiple user personas (Staff, Guest, Admin) across separate interfaces often leads to fragmented state and inconsistent design systems.",
    solution: "Architected a comprehensive monorepo structure featuring distinct apps for Guests, Staff, and Dashboards, integrated with AI-driven services.",
    impact: "Created a scalable, multi-site deployment platform that centralizes core logic while providing tailored experiences for every stakeholder.",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 group">
            ...
            >
              ...
              <div className="absolute top-8 left-8 flex flex-wrap gap-2">
                <span className="px-4 py-1.5 rounded-full bg-primary text-white text-xs font-black uppercase tracking-widest shadow-xl">
                  Featured Deployment
                </span>
              </div>
            </motion.div>
            ...
            <div className="lg:col-span-5 space-y-8 flex flex-col justify-center">
              <div className="space-y-4">
                ...
                <h3 className="text-4xl md:text-5xl font-black tracking-tighter text-foreground group-hover:text-primary transition-colors duration-500 pb-2 leading-tight">{featuredProject.title}</h3>
                
                <div className="space-y-6 pt-4">
                  <div className="space-y-1">
                    <div className="text-xs font-black uppercase tracking-widest text-muted-foreground">The Challenge</div>
                    <p className="text-description text-base leading-relaxed">{featuredDetails.problem}</p>
                  </div>
                  <div className="space-y-1">
                    <div className="text-xs font-black uppercase tracking-widest text-muted-foreground">The Implementation</div>
                    <p className="text-description text-base leading-relaxed">{featuredDetails.solution}</p>
                  </div>
                  <div className="space-y-1">
                    <div className="text-xs uppercase tracking-widest text-primary font-black">Performance Audit</div>
                    <p className="text-foreground text-base font-bold leading-relaxed italic border-l-2 border-primary/30 pl-4">{featuredDetails.impact}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {featuredProject.tags.map(tag => (
                   <span key={tag} className="px-3 py-1 rounded-lg bg-surface-high border border-border text-xs font-black text-muted-foreground uppercase tracking-widest hover:text-primary hover:border-primary/40 transition-all">
                     {tag}
                   </span>
                ))}
              </div>
            ...
            <Reveal key={project.id} delay={0.1 * i} className="h-full">
              ...
              >
                <div className="space-y-8 relative z-10">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-2xl font-black tracking-tighter text-foreground group-hover:text-primary transition-colors duration-500 pb-1 leading-tight">{project.title}</h4>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-2 pt-6 border-t border-border/60">
                    {project.tags.map(tag => (
                      <span key={tag} className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em]">{tag}</span>
                    ))}
                  </div>
                </div>
